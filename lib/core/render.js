...
function render(req, res, view, ext) {
	...
	var get_user_nav = function(){
        ...
        for(var i=0; i<by_list_key.length; i++){
            ...
            list_value = filter_item(list_value);
            if(list_value){
                nav.by.list[list_key] = list_value;
            }
        }
        for(var i=0; i<by_section_key.length; i++){
            ...
            section_value = filter_item(list_value);
            if(section_value){
                nav.by.section[section_key] = section_value;
            }
        }
    	return nav;
	}
